...
DEEP_SCAN=false
VERBOSE=false
GITHUB_TOKEN=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
...
scan_for_secrets() {
    ...
    go run main.go filesystem --results=verified --json "$scan_dir"  >> "$output_file.tmp" || {
        log "${YELLOW}[!] Trufflehog scan failed for $org_repo${NC}"
        return 1
    }
    go run main.go git --results=verified --json file://"$scan_dir"  >> "$output_file.tmp" || {
        log "${YELLOW}[!] Trufflehog scan failed for $org_repo${NC}"
        return 1
    }
